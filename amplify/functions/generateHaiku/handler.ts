  ...
  const input: InvokeModelCommandInput = {
    ...
    body: JSON.stringify({
      ...
      system:
        ...
        "あなたはデータ分析担当です。表形式の画像が送られてくるため、正確に文字情報を読み取ってください。データとデータの間は必ずスペースを開けて、改行してください。もし「値段」の文字があった場合のみ「価格」に変換してください。次の画像をもとにデータを解析結果を表示してください：",
      messages: [
        {
          role: "user",
          content: [...files.map(b64 => ({
            ...
            type: 'text',
            text: '解析結果は、上記のすべてのイメージの組み合わせに基づいて、正確に作成します。「JANコード」は１つの単語です。' 
          }],
        },
      ],
      max_tokens: 5000,
